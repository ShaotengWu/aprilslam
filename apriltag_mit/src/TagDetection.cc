...
namespace AprilTags
{
  ...
  */
    ...
    void TagDetection::draw(cv::Mat &image) const
    {
        ...
        // cv::circle(image, cv::Point2f(p1.first, p1.second), 2, cv::Scalar(0, 255, 0, 0), -1);
        // cv::circle(image, cv::Point2f(p2.first, p2.second), 3, cv::Scalar(0, 255, 0, 0), -1);
        // cv::circle(image, cv::Point2f(p3.first, p3.second), 4, cv::Scalar(0, 255, 0, 0), -1);
        // cv::circle(image, cv::Point2f(p4.first, p4.second), 5, cv::Scalar(0, 255, 0, 0), -1);
